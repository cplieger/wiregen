...
func TestGenerateRegistry(t *testing.T) {
	r := newRegistry()
	out := mustGenNoLoad(t, r.GenerateRegistry)
...
func TestGenerateConstants(t *testing.T) {
	...
	out := mustGenNoLoad(t, r.GenerateConstants)
...
func TestErrorOnEmptyValidatorsImport(t *testing.T) {
	...
	_, err := r.GenerateDecoders(t.Context())
	if err == nil {
...
func TestWithFilenames_CrossImportsFollowTheNames(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			out := mustGenNoLoad(t, newReg(tc.self).GenerateRegistry)
...
// TestGenerate_ContextCancellation pins the one capability the ctx parameter
// exists for: the package load runs the go command as a subprocess, and the
// caller can stop it. It also pins the cancellation REASON surviving in the
// error chain, which the load itself does not guarantee — go/packages formats
// a driver failure into a fresh error without %w, so before newASTEngine
// reported context.Cause explicitly, errors.Is(err, context.Canceled) held on
// only 7 of 20 runs and the other 13 were identifiable by message text alone.
func TestGenerate_ContextCancellation(t *testing.T) {
	newReg := func() *wiregen.Registry {
	...
	t.Run("cancelled", func(t *testing.T) {
		...
		cancel()
		// Repeated because the two error shapes go/packages can return on a
		// cancelled load are racy: a single run cannot show the sentinel is
		// reliable.
		for i := range 20 {
			if _, err := newReg().GenerateTypes(ctx); !errors.Is(err, context.Canceled) {
				t.Fatalf("run %d: GenerateTypes(cancelled ctx) error = %v, want one matching context.Canceled", i, err)
			}
		}
	})
	...
	t.Run("deadlineExceeded", func(t *testing.T) {
		...
		defer cancel()
		if _, err := newReg().GenerateTypes(ctx); !errors.Is(err, context.DeadlineExceeded) {
			t.Errorf("GenerateTypes(expired ctx) error = %v, want one matching context.DeadlineExceeded", err)
		}
	})

	t.Run("cancelCauseReachesTheCaller", func(t *testing.T) {
		abort := errors.New("codegen aborted by the caller")
		ctx, cancel := context.WithCancelCause(t.Context())
		cancel(abort)
		if err := newReg().Generate(ctx, t.TempDir()); !errors.Is(err, abort) {
			t.Errorf("Generate(cause-cancelled ctx) error = %v, want one matching the cause", err)
		}
	})

	t.Run("noWriteOnACancelledGenerate", func(t *testing.T) {
		ctx, cancel := context.WithCancel(t.Context())
		cancel()
		dir := t.TempDir()
		if err := newReg().Generate(ctx, dir); err == nil {
			t.Fatal("Generate(cancelled ctx) error = nil, want non-nil")
		}
		...
		if len(entries) != 0 {
			t.Errorf("Generate(cancelled ctx) wrote %d entries into outDir, want 0", len(entries))
		}
	})
}
...
func TestGenerate_PackageLoadFailureErrors(t *testing.T) {
	...
	if err := r.Generate(t.Context(), t.TempDir()); err == nil {

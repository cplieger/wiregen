...
func TestMapOfUnregisteredStruct(t *testing.T) {
	...
	// is present and required (no omitempty — maps keep source optionality).
	if !strings.Contains(out, "data:") {
		t.Errorf("map field should be present and required, got:\n%s", out)
	}
}
...
func TestTSNameOverride_sanitizesNonIdentifier(t *testing.T) {
	...
	if out := mustGen(t, r.GenerateTypes); !strings.Contains(out, "export interface InnerProfile2") ||
	...
	if out := mustGen(t, r2.GenerateTypes); !strings.Contains(out, "export interface Inner") {
